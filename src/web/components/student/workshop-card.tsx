...
export function WorkshopCard({ workshop }: WorkshopCardProps) {
  ...
  useEffect(() => {
    // Cập nhật đồng hồ đếm ngược mỗi giây nếu có cấu hình thời gian mở/đóng
  ...
  const getButtonConfig = () => {
    ...
    // 1. Kiểm tra chưa đến giờ mở cổng đăng ký -> Hiện countdown, khóa nút
    if (workshop.registrationStartTime) {
      ...
      if (currentTime < startTime) {
        ...
        return {
          ...
          variant: "secondary" as const,
          className: "bg-slate-100 text-slate-500 hover:bg-slate-100 cursor-not-allowed border border-slate-200 font-mono tracking-tight",
          disabled: true
        }
      }
    }
    ...
    if (workshop.registrationEndTime) {
      ...
      if (currentTime > endTime) {
        return {
          ...
          variant: "outline" as const,
          className: "border-slate-300 text-slate-400 bg-slate-50",
          disabled: true
        }
      }
    }
